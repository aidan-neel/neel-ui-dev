<script lang="ts">
  ...
  let items =
    $globalStateStore["combobox"][key]?.items.map((item) => ({ name: item })) ||
    [];
  $: fuse = new Fuse(items, {
    keys: ["name"],
    includeScore: true,
  ...
  $: $globalStateStore["combobox"][key].data.searchPerformed = value.length > 0;
    ...
    if (
      ...
      setTimeout(() => {
        if ($globalStateStore["combobox"][key])
          $globalStateStore["combobox"][key].data.searchPerformed = false;
      }, 150);
    }
  }
</script>
...
<input
  ...
  aria-controls=""
  aria-activedescendant={"combobox-item-" + $globalStateStore["combobox"][key]?.data.selected + key}
  class="w-full p-1 mb-1 pb-2 px-4 placeholder:text-foreground/50 bg-transparent focus:outline-none border-b-default"
  {placeholder}
/>
