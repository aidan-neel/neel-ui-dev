...
{#if open}
  <Blur name="sheet" {key}>
	...
	role="dialog"
	on:mouseenter={() => { entered = true }} on:mouseleave={() => { entered = false }} 
	transition:fly={{ x: side === "left" ? -80 : 80, duration: 250 }}
	{...$$restProps}
	class={cn(`absolute top-0 bg-alternate-muted p-6 h-screen w-[95%] max-w-[30rem] right-0 ${side === "left" ? "border-r-default" : "border-l-default"} ${side}-0`, className)}>
		<slot
			closeSheet={() => {
				globalStateStore.closeStateItem('sheet', key);
			}}
