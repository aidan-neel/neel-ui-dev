...
<div class="flex flex-col gap-2 w-full text-base lg:w-[250px] text-foreground-muted fixed">
    ...
    <div class="flex flex-col gap-1">
      {#if "#" + title.toLowerCase() === hash}
        <a
          ...
          class="text-foreground hover:text-foreground duration-150">{title}</a
        >
      {:else}
        <a
          href={pathName + `#${title.toLowerCase()}`}
      ...
      {#each headings[title] as subtitle}
        {#if "#" + subtitle.toLowerCase() === hash.replace("%20", " ")}
          <a
            ...
            class="ml-4 text-foreground hover:text-foreground duration-150">{subtitle}</a
          >
        {:else}
          <a
            href={pathName + `#${subtitle.toLowerCase()}`}
