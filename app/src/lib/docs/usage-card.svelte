...
<div
  class="flex flex-row gap-4 flex-wrap w-full items-center min-h-[15rem] justify-center rounded-lg border-default"
>
  <div
    class="{SelectedTab === 'preview'
      ? 'flex'
      : 'hidden'} rounded-md bg-alternate-muted w-full h-full flex items-center justify-center"
  >
    <slot />
  </div>
  <div
    class="{SelectedTab === 'code'
      ? 'flex'
      : 'hidden'} rounded-md w-full overflow-auto p-4"
  >
